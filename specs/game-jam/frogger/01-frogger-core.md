...
>
> **Nota de implementación (2026-08-15):** el spec se escribió contra una arquitectura anterior del repo (`FroggerGame.tsx` monolítico, ruta propia `app/games/frogger/play/`, insert client-side con `player_name`/`user_id`). Se implementó la mecánica al 100% sobre el contrato vigente: `components/games/frogger/engine.ts` + `frogger-canvas.tsx` + `registry.ts`, ruta genérica `/games/frogger/jugar`, `saveScoreAction` server action. La fila de catálogo se obtuvo renombrando `ranaria` → `frogger` (no un `INSERT` nuevo), con `color: 'green'` en vez de `'lime'` (token inexistente). Temas visuales (Clásico/Neón/Retro) y controles táctiles añadidos vía `skin-designer`/`mobile-porter` tras el core. Detalle completo en el plan de implementación de la sesión.
